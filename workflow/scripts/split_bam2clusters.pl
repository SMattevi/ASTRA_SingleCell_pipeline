...
print $cluster;
  ...
  open(BAMHEAD, "samtools view -H $bam_file |" ) or die("Cannot read header of $bam_file \n");
  while(my $hh = <BAMHEAD>)
    {
...
open(BAM, "samtools view $bam_file |" ) or die("Cannot read $bam_file \n");
...
foreach my $cluster (keys %cluster_files)
{
  ...
  system("samtools view -bS $cluster_file > $cluster_bam_file");
